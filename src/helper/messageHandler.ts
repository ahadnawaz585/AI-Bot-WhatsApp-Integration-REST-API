...
export class MessageHandler {
    ...
    private async startMessageProcess(message: Message, client: Client): Promise<void> {
        await client.sendMessage(message.from, config.welcomeMessage);
        this.shouldProcessMessages = true;
    }

    private async endMessageProcess(message: Message, client: Client): Promise<void> {
        await client.sendMessage(message.from, config.closingMessage);
        this.shouldProcessMessages = false;
    }

    private async processMessages(message: Message, client: Client): Promise<void> {
  ...
  private async reply(prompt: string, model?: string, imagePaths?: Vision[]): Promise<string> {
    let reply: string;
    ...
    switch (model) {
        case "!gemini":
            ...
            break;
        case "!vision":
            const visionResponse =  await this.vision.generateContent(prompt, imagePaths || []);
